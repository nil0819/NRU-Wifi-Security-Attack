...
void
BackoffChangeCb (std::string context, uint32_t value)
{
  BackoffChange bc;
...
void
LteTxopReceived (std::string context, Time startTime, Time duration, Time nextSubframeStarts)
{
  HandleTxopReceived (context, startTime, duration);
}

void
WifiTxopReceived (std::string context, Time startTime, Time duration)
{
  HandleTxopReceived (context, startTime, duration);
}
...
void
ScheduleTxopLogConnect (Config_e cellConfigA, Config_e cellConfigB)
{
...
void
ScheduleTxopLogDisconnect (Config_e cellConfigA, Config_e cellConfigB)
{
  if (cellConfigA == WIFI || cellConfigB == WIFI)
    {
...
SaveBackoffStats (std::string filename, const std::vector<BackoffChange> &changes)
{
  ...
  outFile.setf (std::ios_base::fixed);
  outFile << "#time(s) nodeId backoff" << std::endl;
                         ...
                         std::string simulationParams)
{
  ...
  bool wifiUsed = ((cellConfigA == WIFI) || (cellConfigB == WIFI));
  ...
  if (booleanValue.Get ())
    {
      Simulator::Schedule (clientStartTime, &ScheduleTxopLogConnect, cellConfigA, cellConfigB);
      //Simulator::Schedule (clientStopTime, &ScheduleTxopLogDisconnect, cellConfigA, cellConfigB);
    }
  ...
  if (booleanValue.Get ())
    {
      Time txopDurationCounterA = Seconds (0), txopDurationCounterB = Seconds (0);
      for (const auto &txopLog : g_txopLogs)
        {
          if (txopLog.m_nodeId % 2 == 0) // cell A
            {
              txopDurationCounterA += txopLog.m_duration;
            }
          else // cell B
            {
