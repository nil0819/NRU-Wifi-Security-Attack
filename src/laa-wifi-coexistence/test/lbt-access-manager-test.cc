...
protected:
  ...
  void SpectrumWifiPhyRxSuccess (Ptr<WifiPsdu> p, double snr, WifiTxVector txVector, std::vector<bool> perMpduStatus);
  void SpectrumWifiPhyRxFailure (Ptr<WifiPsdu> p);
  uint32_t m_count;
...
LbtAccessManagerBaseTestCase::MakeSignal (double txPowerWatts)
{
  ...
  hdr.SetQosTid (0);
  uint32_t size = pkt->GetSize () + hdr.GetSize () + WIFI_MAC_FCS_LENGTH;
  Time txDuration = m_phy->CalculateTxDuration (size, txVector, WifiPhyBand::WIFI_PHY_BAND_5GHZ);
  NS_LOG_DEBUG ("Signal duration " << txDuration.GetSeconds ());
  hdr.SetDuration (txDuration);
  Ptr<WifiPsdu> psdu = Create<WifiPsdu> (pkt, hdr);
  Ptr<WifiPpdu> ppdu = Create<WifiPpdu> (psdu, txVector, txDuration, WifiPhyBand::WIFI_PHY_BAND_5GHZ);
...
void
LbtAccessManagerBaseTestCase::SpectrumWifiPhyRxSuccess (Ptr<WifiPsdu> p, double snr, WifiTxVector txVector, std::vector<bool> perMpduStatus)
{
...
void
LbtAccessManagerBaseTestCase::SpectrumWifiPhyRxFailure (Ptr<WifiPsdu> p)
{
...
LbtDeferAndBackoff::DoRun (void)
{
  ...
  // The LBT should be busy until 7001431, then defer for 43, then backoff
  // But interrupt the defer at 7001431 with another signal, which holds it busy
  // again until 7002982
  Simulator::Schedule (MicroSeconds (7001431),  &LbtAccessManagerBaseTestCase::SendSignal, this, txPowerWatts);
  ...
  int64_t expectedExpiration1 = 7001431 + 1400 + 43 + 9 * m_backoff[1];
...
LbtSuspendBackoff::DoRun (void)
{
  ...
  Simulator::Schedule (MicroSeconds (8001474), &LbtSuspendBackoff::GetBackoff, this);
  ...
  NS_TEST_ASSERT_MSG_EQ (m_backoff[0] - m_backoff[1], 2,  "Backoff didn't decrement by two");
